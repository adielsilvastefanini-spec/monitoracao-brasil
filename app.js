...
function fn02_reordenarTabela() {
  ...
  $rows.sort(function(a, b) {
    ...
    return getPeso($tdA) - getPeso($tdB);
  });
...
function fn03_avaliarStatusLinha($tr) {
  ...
  if (tipo === 'ATIVIDADE PROGRAMADA' || tipo === 'MANUTENÇÃO') {
    $tdSitio.addClass('sitio-cinza');
  } else if (data1 && hora1 && data2 && hora2) {
...
function fn04_carregarTiposPorSitio($tr, sitio) {
  var $selectTipo = $tr.find('.select-tipo');
  var valAtual = $selectTipo.val();
  $selectTipo.html('<option value="">Selecione...</option>');

  if (sitio && dadosSitios[sitio]) {
    Object.keys(dadosSitios[sitio]).sort().forEach(function(t) {
      $selectTipo.append(`<option value="${t}">${t}</option>`);
    });
  }
  if (valAtual) $selectTipo.val(valAtual);
}

function fn05_carregarParceirosPorTipo($tr, sitio, tipo) {
  var $selectParceiro = $tr.find('.select-parceiro');
  var valAtual = $selectParceiro.val();
...
function fn08_criarLinhaTabela(idRow) {
  var optionsSitios = fn01_getOptionsSitio();
  var trHTML = `
    <tr data-id="${idRow}">
      <td class="text-center"><input type="checkbox" class="check-item"></td>
      <td class="col-sitio">
        <select class="form-select form-select-sm select-sitio">${optionsSitios}</select>
      </td>
      <td><select class="form-select form-select-sm select-tipo"><option value="">Selecione...</option></select></td>
      <td><input type="date" class="form-control form-control-sm input-data1"></td>
      <td><input type="time" class="form-control form-control-sm input-hora1"></td>
      <td><input type="date" class="form-control form-control-sm input-data2"></td>
      <td><input type="time" class="form-control form-control-sm input-hora2"></td>
      <td>
      ...
      <td><select class="form-select form-select-sm select-parceiro"><option value="">Selecione...</option></select></td>
      <td><input type="text" class="form-control form-control-sm input-ticket"></td>
      <td><input type="text" class="form-control form-control-sm input-status"></td>
    </tr>
  `;
  var $tr = $(trHTML);
  $('#incidentes tbody').append($tr);
  return $tr;
}
